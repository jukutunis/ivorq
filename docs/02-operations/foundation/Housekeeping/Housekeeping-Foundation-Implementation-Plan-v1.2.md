...
## 19. Linen Workspace Architecture

**Overview:** Comprehensive lifecycle tracking specifically targeting the flow of linens from rooms to laundry facilities and back to storage.
*   **Laundry Queue:** Tracks bulk volume of soiled linens collected from floors waiting to be dispatched to the internal or external laundry vendor.
*   **Outgoing Linen:** Manifest generation and batch ID tracking when linens leave the property or enter the washing cycle.
*   **Incoming Linen:** Receiving workflow verifying returned quantities against the outgoing manifest to identify missing items.
*   **Damaged Linen:** Dedicated quarantine flow for heavily stained or torn linens to be discarded or repurposed (e.g., as cleaning rags), automatically deducting from inventory via the Inventory Foundation.
*   **Vendor Laundry Tracking:** Tracks turnaround times, billing reconciliation data, and loss ratios per external vendor.
*   **Linen Variance Management:** Automated variance calculation mapping discrepancies between theoretical usage and actual laundry returns.

## 20. Turndown Service Engine

**Overview:** Evening service scheduling and execution for premium properties or VIP rooms.
*   **Turndown Task:** Distinct from standard cleaning tasks; requires separate scheduling (typically evening shifts) and targets specific occupied rooms.
*   **Turndown Checklist:** Specialized lightweight checklist focusing on bed preparation, lighting, drawing curtains, and placing evening amenities.
*   **Turndown SLA:** Stricter timing constraints based on guest dinner reservations or preferred service times.
*   **Turndown Scoring:** Unique inspection metrics focusing heavily on presentation and ambiance rather than deep cleanliness.

## 21. Housekeeping Credit System

**Overview:** Workload management system translating physical rooms and tasks into standard operational "credits" or "points".
*   **Credit Based Assignment:** Rooms are assigned variable credit values (e.g., a standard stayover might be 1 credit, a suite departure might be 2.5 credits). Attendants are assigned a maximum daily credit cap (e.g., 15 credits).
*   **Workload Balancing:** The assignment engine automatically distributes tasks to prevent burnout and ensure equitable labor distribution across the team.
*   **Productivity Metrics:** Tracks "credits completed per hour" for individual and departmental performance reviews.
*   **Payroll Integration Readiness:** Structures data to allow future integration with HRIS or Payroll modules, enabling piece-rate or productivity-bonus compensation models.

## 22. Supervisor Mobile Workspace

**Overview:** A dedicated mobile interface for floor supervisors to conduct QA and manage their zones dynamically.
*   **Inspection Queue:** Real-time list of rooms marked `CLEAN` by attendants, prioritized by the Room Readiness Engine.
*   **Failed Inspection Queue:** Dashboard tracking rooms that require rework, monitoring how quickly attendants rectify the issues.
*   **Pickup Queue:** Quick tasks (e.g., fixing a minor detail) that the supervisor can assign or resolve themselves without dispatching an attendant.
*   **Reinspection Workflow:** A secondary, stricter QA process for rooms that previously failed inspection.
*   **Lost & Found Approval:** Supervisor sign-off workflow for high-value items discovered by attendants before they are transferred to security.
*   **Amenity Audit:** Mobile interface for conducting spot checks on pantry carts and floor storage to maintain inventory integrity.

## 23. Executive Housekeeper Dashboard

**Overview:** High-level strategic and analytical view for department heads.
*   **Productivity Analytics:** Aggregate views of team credit completion, time-to-clean averages, and outlier identification.
*   **SLA Analytics:** Global adherence to turnaround times and VIP readiness targets.
*   **Inspection Analytics:** Heatmaps of common failure points (e.g., "Dust on skirting boards") across the property or per attendant.
*   **Linen Cost Analytics:** Tracks replacement rates, laundry vendor billing accuracy, and overall linen lifespan ROI.
*   **Amenity Cost Analytics:** Usage trends per occupied room, highlighting over-consumption or potential shrinkage.
*   **Room Readiness Analytics:** Daily graphs showing the curve of room availability against the curve of guest arrivals.
*   **Labor Utilization:** Real-time visibility into active hours vs. productive cleaning hours to optimize scheduling.

## 24. PMS Adapter Architecture

**Overview:** A standardized integration layer allowing the Housekeeping Foundation to communicate agnostically with various external Property Management Systems.
*   **RMS Cloud Adapter:** Standard integration mapping.
*   **Opera Adapter:** Legacy/Enterprise integration mapping.
*   **Mews Adapter:** Modern cloud webhook integration.
*   **Cloudbeds Adapter:** Mid-market API integration.
*   **HotelTime Adapter:** Regional API integration.
*   **Generic PMS Event Contract:** An internal Event-Driven architecture (e.g., `GuestCheckedOut`, `RoomStatusChanged`) that insulates the core Housekeeping logic from PMS-specific API quirks.

## 25. Expand Room Readiness Engine

**Overview:** Extends the readiness logic to account for granular operational states beyond simple clean/dirty binaries.
*   **Ready For Sale:** Room is `INSPECTED`, mechanically sound, and completely unassigned in the PMS.
*   **Ready For Arrival:** Room is `INSPECTED` and specifically pre-assigned to an arriving guest whose ETA is approaching.
*   **Ready For VIP:** Room is `INSPECTED`, has received a secondary executive inspection, and has custom VIP amenities placed.
*   **Waiting Inspection:** Room is physically `CLEAN` but blocked from front desk assignment until a supervisor signs off.
*   **Waiting Engineering:** Room cleaning is paused or blocked because an active `WorkOrder` is pending in the Engineering Foundation.
*   **Waiting Amenities:** Room is clean but lacking specific inventory items (e.g., special request pillows) required before it can be marked ready.
*   **Blocked:** Room is `OOS` or `OOO` and completely removed from the readiness queue.
