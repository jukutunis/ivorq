...
| Night Audit and hospitality business-date foundation | Approved | Architecture activated on 2026-07-16; BD-A1 authoritative Property Business Date runtime, FD-B11 read integration, NA-A1 Night Audit run/active-lock foundation, and FD-B12 read-only Front Desk lock integration are accepted. FD-B12 is canonical at `286c7f491ea82385ee369ff0020625448eea671d`. FD-B13 is the current checkout execution readiness review package. This is not full Night Audit runtime. | ADR-034 |
| PMS readiness | Partial accepted foundations | PMS Guest Ledger, PMS Cashiering, Front Desk, General Cashier, GLF-D, FD-B9, GC-A1, FD-B10, BD-A1, FD-B11, NA-A1, and FD-B12 dependencies are accepted per the current contract baseline. FD-B13 is the current review package. Checkout implementation remains locked, no runtime checkout command has been introduced, and the FD-B13 verdict is `CHECKOUT_EXECUTION_BLOCKED_BY_PREREQUISITES`. | ADR-034, ADR-087, ADR-088 |
...
**PMS Foundation**: Partial Accepted Foundations, BD-A1 Business Date Runtime Accepted, FD-B11 Accepted, NA-A1 Night Audit Run/Lock Foundation Accepted, FD-B12 Front Desk Lock Read Integration Accepted at `286c7f491ea82385ee369ff0020625448eea671d`, FD-B13 Checkout Execution Readiness Review Current
...
The current ADR portfolio is architecturally sufficient to continue controlled Finance, Security, Privacy, Procurement, Tax, and package-scoped PMS/Business Date work. It is not a claim of full PMS production readiness or implementation completion. BD-A1, FD-B11, NA-A1, and FD-B12 are accepted; FD-B12 is canonical at `286c7f491ea82385ee369ff0020625448eea671d`. FD-B13 is the current review package and records verdict `CHECKOUT_EXECUTION_BLOCKED_BY_PREREQUISITES`. Prerequisite package categories include Night Audit / checkout shared concurrency guard, PMS Guest Ledger terminal financial attestation/freeze, General Cashier obligation terminalization, Front Desk immutable checkout execution evidence, transactional checkout handoff/outbox, Sensitive Action Confirmation intent registration, and the later checkout execution permission/command package. Checkout implementation, Business Date close/advance/reopen, Night Audit checkpoints, and runtime checkout command activation remain locked or separately unauthorized. No runtime checkout command has been introduced. No new ADR is created by this package. Future ADR creation remains trigger-based and requires separate Owner authorization.
