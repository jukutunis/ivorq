...
|---|---|---|---|---|---|---|---|
| **MPD-FND** - Foundation, tenancy, Property, identity, organizational foundation | `Modules/Foundation/{Property,User,Department,Authentication,Authorization,Audit,Approval,Outbox}`; substantial source (`SOURCE_PROVEN`) | ADR-001, ADR-002, ADR-003, ADR-030, ADR-090; `docs/audits/Property-Isolation-Audit.md` | Foundation source remains active; ADR-090 approves the operational first-trust architecture only and leaves runtime implementation, privileged-human activation, and B4C linkage to separately authorized packages; general knowledge-base material includes archived documents and requires source revalidation (`INFERENCE`) | `SOURCE_PRESENT_REVALIDATION_REQUIRED` | SEC, every Property-scoped operational domain | `FND-R1` | P2 |
| **MPD-SEC** - Security, Authorization, Privacy, Data Governance | `Modules/Foundation/{Authentication,Authorization,Audit,User}` and shared policy/session controls (`SOURCE_PROVEN`) | ADR-002, ADR-029, ADR-030, ADR-031, ADR-066, ADR-090; `docs/security/` | Sensitive confirmation and Housekeeping/Finance permission controls have accepted runtime provenance; ADR-090 governs first trust without changing ADR-030 from Proposed or claiming verification/MFA runtime readiness; the broader security/privacy portfolio is not canonically closed (`GOVERNANCE_PROVEN`) | `SOURCE_PRESENT_REVALIDATION_REQUIRED` | FND and all domains; especially FIN, PMS, HKG | `SEC-R1` | P1 |
