...
class RealizedFxAdjustmentFinalizationAuthorizationTest extends PostgresTestCase
{
    use RefreshDatabase;
    ...
    public function test_can_authorize_valid_draft(): void
    {
        ...
        $authorized = $this->assertNoExternalMutations(function () use ($draft) {
            return $this->authorizeService->authorize($draft->id, $this->authorizer->id);
        });

        $this->assertSame($this->authorizer->id, $authorized->draft_finalization_authorized_by);
        $this->assertNotNull($authorized->draft_finalization_authorized_at);
        $this->assertSame(JournalStatusEnum::Draft, $authorized->status); // Status remains Draft, not Posted
    }

    public function test_self_authorization_is_permitted_by_default_source(): void
    {
        $context = $this->makeSettlementContext();
        $candidate = $this->candidateService->create($context['allocation_id'], $this->creator);
        $this->reviewService->approve($candidate->id, $this->reviewer->id);
        ...
        $this->materializer->givePermissionTo(RealizedFxAdjustmentFinalizationAuthorizationService::PERMISSION);
        app(PermissionRegistrar::class)->forgetCachedPermissions();

        $authorized = $this->assertNoExternalMutations(function () use ($draft) {
            return $this->authorizeService->authorize($draft->id, $this->materializer->id);
        });

        $this->assertSame($this->materializer->id, $authorized->draft_finalization_authorized_by);
    }

    public function test_missing_permission_or_property_fails_closed(): void
    {
        $context = $this->makeSettlementContext();
        $candidate = $this->candidateService->create($context['allocation_id'], $this->creator);
        $this->reviewService->approve($candidate->id, $this->reviewer->id);
        ...
        $this->authorizeService->authorize($draft->id, $badActor->id);
    }

    public function test_invalid_source_type_draft_rejected(): void
    {
        // Materialize a standard non-FX draft
        $id = (string) Str::ulid();
        ...
        $this->expectExceptionMessage('Only realized FX drafts can be finalization-authorized.');

        $this->authorizeService->authorize($id, $this->authorizer->id);
    }

    public function test_idempotent_identical_replay(): void
    {
        $context = $this->makeSettlementContext();
        $candidate = $this->candidateService->create($context['allocation_id'], $this->creator);
        $this->reviewService->approve($candidate->id, $this->reviewer->id);
        ...
        $first = $this->authorizeService->authorize($draft->id, $this->authorizer->id);
        $second = $this->authorizeService->authorize($draft->id, $this->authorizer->id);

        $this->assertSame($first->id, $second->id);
    }

    public function test_conflicting_replay(): void
    {
        $context = $this->makeSettlementContext();
        $candidate = $this->candidateService->create($context['allocation_id'], $this->creator);
        $this->reviewService->approve($candidate->id, $this->reviewer->id);
        ...
        $this->authorizeService->authorize($draft->id, $this->authorizer->id);

        $otherAuthorizer = $this->makeUser();
        $this->attachActorToProperty($otherAuthorizer, $this->property);
        $otherAuthorizer->givePermissionTo(RealizedFxAdjustmentFinalizationAuthorizationService::PERMISSION);
        app(PermissionRegistrar::class)->forgetCachedPermissions();

        $this->expectException(\RuntimeException::class);
        $this->expectExceptionMessage('Conflicting JournalEntry draft finalization authorization evidence already exists.');

        $this->authorizeService->authorize($draft->id, $otherAuthorizer->id);
    }
