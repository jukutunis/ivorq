...
class RealizedFxAdjustmentPostingTest extends PostgresTestCase
{
    use RefreshDatabase;
    ...
    private RealizedFxAdjustmentFinalizationAuthorizationService $authorizeService;
    private RealizedFxAdjustmentPostingService $postingService;
    ...
    protected function setUp(): void
    {
        ...
        $this->authorizer = $this->makeUser();
        $this->poster = $this->makeUser();
        ...
        $this->authorizeService = app(RealizedFxAdjustmentFinalizationAuthorizationService::class);
        $this->postingService = app(RealizedFxAdjustmentPostingService::class);
    }
    ...
    public function test_can_post_authorized_gain_draft(): void
    {
        ...
        $draft = $this->materializeService->materialize($candidate->id, $this->materializer->id);
        $authorized = $this->authorizeService->authorize($draft->id, $this->authorizer->id);

        $posted = $this->assertNoExternalMutations(function () use ($authorized) {
            return $this->postingService->post($authorized->id, $this->poster->id);
        });

        $this->assertSame(JournalStatusEnum::Posted, $posted->status);
        $this->assertSame($this->poster->id, $posted->posted_by);
        $this->assertNotNull($posted->posted_at);
        $this->assertStringStartsWith(now()->toDateString(), (string)$posted->posting_date);

        // Verify candidate status is now POSTED
        $freshCandidate = JournalCandidate::findOrFail($candidate->id);
        $this->assertSame(JournalCandidateStatusEnum::POSTED, $freshCandidate->status);

        // Verify ledger balances updated correctly
        $apBalance = DB::table('gl_ledger_balances')
            ->where('property_id', $this->property->id)
            ->where('account_id', $this->apAccountId)
            ->first();

        // AP Control had credit of 125, debit of 120. Now we debit another 5.00
        // ending balance in LedgerBalance represents sum of debits and credits
        // normal balance for AP-CTRL is Credit, so ending_balance = credit_total - debit_total
        $this->assertSame('5.00', (string)$apBalance->debit_total);
        $this->assertSame('0.00', (string)$apBalance->credit_total);
    }

    public function test_can_post_authorized_loss_draft(): void
    {
        $context = $this->makeSettlementContext(
            carryingBasis: '125.00',
    ...
    public function test_missing_authorization_fails_closed(): void
    {
        $context = $this->makeSettlementContext();
        $candidate = $this->candidateService->create($context['allocation_id'], $this->creator);
        $this->reviewService->approve($candidate->id, $this->reviewer->id);
        ...
        $this->expectExceptionMessage('JournalEntry draft must be finalization-authorized before posting.');

        $this->postingService->post($draft->id, $this->poster->id);
    }
    ...
    public function test_actor_permission_property_membership_failures(): void
    {
        ...
        $this->postingService->post($authorized->id, $badActor->id);
    }

    public function test_closed_financial_period_fails_closed(): void
    {
        $context = $this->makeSettlementContext();
        $candidate = $this->candidateService->create($context['allocation_id'], $this->creator);
        $this->reviewService->approve($candidate->id, $this->reviewer->id);
        ...
        $this->expectExceptionMessage('is closed or closing');

        $this->postingService->post($authorized->id, $this->poster->id);
    }
    ...
    public function test_idempotent_posting_replay(): void
    {
        $context = $this->makeSettlementContext();
        $candidate = $this->candidateService->create($context['allocation_id'], $this->creator);
        $this->reviewService->approve($candidate->id, $this->reviewer->id);
        ...
        $first = $this->postingService->post($authorized->id, $this->poster->id);
        $second = $this->postingService->post($authorized->id, $this->poster->id);

        $this->assertSame($first->id, $second->id);
    }

    public function test_conflicting_posting_replay(): void
    {
        $context = $this->makeSettlementContext();
        $candidate = $this->candidateService->create($context['allocation_id'], $this->creator);
        $this->reviewService->approve($candidate->id, $this->reviewer->id);
        ...
        $otherPoster = $this->makeUser();
        $this->attachActorToProperty($otherPoster, $this->property);
        $otherPoster->givePermissionTo(RealizedFxAdjustmentPostingService::PERMISSION);
        app(PermissionRegistrar::class)->forgetCachedPermissions();

        $this->expectException(\RuntimeException::class);
        $this->expectExceptionMessage('Conflicting JournalEntry posting evidence already exists.');

        $this->postingService->post($authorized->id, $otherPoster->id);
    }
