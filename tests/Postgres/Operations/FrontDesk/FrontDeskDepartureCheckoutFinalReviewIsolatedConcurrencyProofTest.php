...
class FrontDeskDepartureCheckoutFinalReviewIsolatedConcurrencyProofTest extends PostgresTestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->setUpConcurrencyDatabase('ivorq_concurrency_fd_b7_', '2026-07-10 11:00:00');
    ...
    private function seedB7Data(): void
    {
        ...
        app(FrontDeskDepartureCheckoutAuthorizationService::class)->create($this->frontDeskActor, $s[0]->id, 'CHECKOUT_AUTHORIZATION_READY', null, 'dca-seed-' . Str::ulid());
        $this->stayId = $s[0]->id;
    ...
    public function test_idempotent_concurrency(): void
    {
        ...
        $this->simCreate('A', $k, 'CHECKOUT_FINAL_REVIEW_READY');
        $this->simCreate('B', $k, 'CHECKOUT_FINAL_REVIEW_READY');
        $this->assertSame(1, FrontDeskDepartureCheckoutFinalReview::on('pgsql_concurrency')->withoutGlobalScopes()->where('front_desk_stay_id', $this->stayId)->count());
    }
    ...
    public function test_distinct_concurrency(): void
    {
        $k1 = 'dist-1-' . Str::ulid();
        $k2 = 'dist-2-' . Str::ulid();
        $this->simCreate('A', $k1, 'CHECKOUT_FINAL_REVIEW_READY');
        $this->simCreate('B', $k2, 'CHECKOUT_FINAL_REVIEW_BLOCKED');
    ...
    private function simCreate(string $w, string $k, string $s): void
    {
        ...
        try {
            ...
            app(FrontDeskDepartureCheckoutFinalReviewService::class)->create($a, $this->stayId, $s, null, $k);
