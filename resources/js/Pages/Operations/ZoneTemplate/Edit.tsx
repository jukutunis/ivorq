...
import { ZoneTemplate, EnumOption } from '@/Types';
...
export default function ZoneTemplateEdit({ template, zone_types, priorities }: Props) {
    const { data, setData, put, processing, errors } = useForm({
        template_name:    template.template_name,
        zone_type:        String(template.zone_type.value),
        default_priority: String(template.default_priority.value),
        ...
        <AppLayout>
            ...
            <div className="flex items-center justify-between mb-6">
                ...
                {template.is_active ? (
                    ...
                    <span className="inline-flex items-center px-2.5 py-0.5 rounded text-xs font-medium bg-gray-100 text-gray-500">Inactive</span>
                )}
            </div>
            ...
            <form onSubmit={submit} className="max-w-2xl">
                ...
                <div className="flex items-center gap-3 mt-4">
                    ...
                    <Link
                        href={`/operations/zone-templates/${template.id}`}
                        method="delete"
                        as="button"
                        className="ml-auto text-red-600 hover:text-red-800 text-sm px-3 py-2"
                        onBefore={() => confirm('Delete this template? This action cannot be undone.')}
                    >
                        Delete Template
                    </Link>
                </div>
