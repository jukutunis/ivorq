...
interface ExistingLine { id: string; item_id: string; quantity_requested: number; notes: string | null; }
interface Transfer {
    id: string; transfer_number: string;
    from_location_id: string; to_location_id: string;
    status: EnumOption; notes: string | null; lines?: ExistingLine[];
}
interface LineForm { item_id: string; quantity_requested: string; notes: string; }
interface Props { transfer: Transfer; items: InventoryItem[]; locations: InventoryLocation[]; }

const emptyLine = (): LineForm => ({ item_id: '', quantity_requested: '', notes: '' });

export default function TransferEdit({ transfer, items, locations }: Props) {
    const existingLines: LineForm[] = (transfer.lines ?? []).map((l) => ({
        item_id: l.item_id, quantity_requested: String(l.quantity_requested), notes: l.notes ?? '',
    }));
    ...
    function submit(e: React.FormEvent) {
        e.preventDefault();
        put(`/operations/inventory/transfers/${transfer.id}`);
    }

    return (
        <AppLayout>
            <div className="flex items-center gap-3 mb-6">
                <Link href={`/operations/inventory/transfers/${transfer.id}`} className="text-sm text-gray-500 hover:text-gray-700">← {transfer.transfer_number}</Link>
            </div>
            ...
            <form onSubmit={submit} className="max-w-4xl space-y-6">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <Link href={`/operations/inventory/transfers/${transfer.id}`} className="bg-gray-100 text-gray-700 px-5 py-2 rounded text-sm hover:bg-gray-200">Cancel</Link>
                </div>
            </form>
        </AppLayout>
    );
