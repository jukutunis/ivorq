...
export default function UnitShow({ unit }: Props) {
    ...
    function destroy() {
        if (confirm(`Delete unit "${unit.name}"? This cannot be undone.`)) {
            router.delete(`/operations/inventory/units/${unit.id}`);
        }
    }
        ...
        <AppLayout>
            ...
            <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                    ...
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded text-xs font-medium ${unit.is_active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
                        {unit.is_active ? 'Active' : 'Inactive'}
                    </span>
                </div>
                <div className="flex gap-2">
                    {can('inventory.unit.edit') && (
                        <Link href={`/operations/inventory/units/${unit.id}/edit`}
                            className="bg-gray-100 text-gray-700 px-4 py-2 rounded text-sm hover:bg-gray-200">Edit</Link>
                    )}
                    {can('inventory.unit.delete') && (
            ...
            <div className="bg-white rounded-lg shadow p-6">
                <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-4">Unit Details</h2>
                <div className="grid grid-cols-2 gap-6 md:grid-cols-4">
                    <div>
                        <p className="text-xs text-gray-500 mb-1">Unit Code</p>
                        <p className="text-sm font-mono text-gray-700">{unit.unit_code}</p>
                    </div>
                    <div>
                    ...
                    <div>
                        <p className="text-xs text-gray-500 mb-1">Abbreviation</p>
                        <p className="text-sm font-mono text-gray-700">{unit.abbreviation}</p>
                    </div>
                    <div>
                        <p className="text-xs text-gray-500 mb-1">Status</p>
                        <p className="text-sm text-gray-700">{unit.is_active ? 'Active' : 'Inactive'}</p>
                    </div>
                </div>
