...
import { InventoryUnit, PaginatedData, PageProps } from '@/Types';

interface Props {
    units:   PaginatedData<InventoryUnit>;
...
export default function UnitIndex({ units, filters }: Props) {
    ...
    function applyFilter(field: string, value: string) {
        router.get('/operations/inventory/units', { ...filters, [field]: value || undefined }, { preserveState: true });
    }
        ...
        <AppLayout>
            <div className="flex items-center justify-between mb-6">
                <div>
                    ...
                    <p className="text-sm text-gray-500 mt-1">{units.total} unit{units.total !== 1 ? 's' : ''} total</p>
                </div>
                <div className="flex flex-wrap gap-2">
                    ...
                    {can('inventory.unit.create') && (
                        <Link href="/operations/inventory/units/create" className="bg-blue-600 text-white px-4 py-2 rounded text-sm hover:bg-blue-700">New Unit</Link>
                    )}
                </div>
            </div>
            ...
            <div className="bg-white rounded-lg shadow overflow-hidden">
                {units.data.length === 0 ? (
                    <div className="px-6 py-12 text-center text-gray-400 text-sm">No units found.</div>
                ) : (
                        ...
                        <table className="w-full text-sm">
                            <thead className="bg-gray-50 border-b border-gray-200">
                                <tr>
                                    ...
                                    <th className="text-left px-6 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider">Abbreviation</th>
                            ...
                            <tbody className="divide-y divide-gray-100">
                                {units.data.map((unit) => (
                                    <tr key={unit.id} className="hover:bg-gray-50">
                                        <td className="px-6 py-4 font-mono text-xs text-gray-600">{unit.unit_code}</td>
                                        <td className="px-6 py-4 font-medium text-gray-900">{unit.name}</td>
                                        <td className="px-6 py-4 text-gray-600 font-mono text-xs">{unit.abbreviation}</td>
                                        <td className="px-6 py-4">
                                            <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${unit.is_active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
                                                {unit.is_active ? 'Active' : 'Inactive'}
                                            </span>
                                        </td>
                                        <td className="px-6 py-4 text-right">
                                            <Link href={`/operations/inventory/units/${unit.id}`} className="text-blue-600 hover:text-blue-800 text-sm">View</Link>
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
