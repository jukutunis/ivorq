...
class RealizedFxAdjustmentCandidateService
{
     ...
     */
    public function create(string $allocationId, ?User $actor): JournalCandidate
    {
        return DB::transaction(function () use ($allocationId, $actor): JournalCandidate {
            ...
            if ($comp === 0) {
                throw new DomainException('Realized FX difference is zero; no candidate is required.');
            }
            ...
            // 9. Enforce Idempotency (prevent duplicate/conflicting source tuple candidates)
