...
class RealizedFxAdjustmentCandidateReviewService
{
    public const PERMISSION = 'finance.journal-candidate.review';

    /**
     * Approve a PENDING_REVIEW realized FX candidate.
     */
    public function approve(string $id, string $userId): JournalCandidate
    {
        return DB::transaction(function () use ($id, $userId) {
            ...
            $this->assertNotSelfReview($candidate, $user);
            $this->assertNotAdvancedState($candidate);

            // Safe idempotent retry / conflict check
    ...
    /**
     * Reject a PENDING_REVIEW realized FX candidate with a reason.
     */
    ...
    private function assertRealizedFxCandidate(JournalCandidate $candidate): void
    {
        if ($candidate->source_type !== 'ApSettlementAllocation' ||
            ...
            throw new \RuntimeException('Candidate does not belong to the realized FX source type.');
        }
    }

    private function assertNotSelfReview(JournalCandidate $candidate, User $user): void
    {
        if ($candidate->created_by === $user->id) {
            throw new AuthorizationException('Self-review is strictly prohibited.');
        }
    }

    private function assertNotAdvancedState(JournalCandidate $candidate): void
    {
        if ($candidate->status === JournalCandidateStatusEnum::POSTED ||
            $candidate->status === JournalCandidateStatusEnum::POSTED_LEGACY) {
            throw new \RuntimeException('Candidate has already been posted.');
        }

        $draftExists = DB::table('gl_journal_entries')
            ->where('journal_candidate_id', $candidate->id)
            ->exists();

        if ($draftExists) {
            throw new \RuntimeException('Candidate has already been materialized into a draft journal entry.');
        }
    }
