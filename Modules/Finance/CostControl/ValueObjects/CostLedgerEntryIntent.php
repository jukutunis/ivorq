...
class CostLedgerEntryIntent
{
    ...
    public function __construct(
        ...
        if (empty($propertyId)) {
            throw new InvalidArgumentException("propertyId cannot be empty");
        }
        if (empty($sourceInventoryTransactionId)) {
            throw new InvalidArgumentException("sourceInventoryTransactionId cannot be empty");
        }
        if ($entrySequence <= 0) {
            throw new InvalidArgumentException("entrySequence must be positive");
        }
        if (empty($entryType)) {
            throw new InvalidArgumentException("entryType cannot be empty");
        }
        if (empty($idempotencyKey)) {
            throw new InvalidArgumentException("idempotencyKey cannot be empty");
        }
        if (empty($currencyCode) || strlen($currencyCode) !== 3) {
            throw new InvalidArgumentException("currencyCode must be a 3-character string");
        }
        if (empty($businessDate)) {
            throw new InvalidArgumentException("businessDate cannot be empty");
        }
        if (empty($occurredAt)) {
